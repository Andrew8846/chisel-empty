...
class ForwardingUnit extends Module {
  val io = IO(new Bundle {
    ...
    val wbWriteEn = Input(Bool())

    // Forwarding select signals for operands
    val forwardA  = Output(UInt(2.W))
    val forwardB  = Output(UInt(2.W))

  })

  val exHazardA = (io.exRd =/= 0.U) && (io.exRd === io.idRs1)
  val exHazardB = (io.exRd =/= 0.U) && (io.exRd === io.idRs2)
  val wbHazardA = io.wbWriteEn && (io.wbRd =/= 0.U) && (io.wbRd === io.idRs1)
  val wbHazardB = io.wbWriteEn && (io.wbRd =/= 0.U) && (io.wbRd === io.idRs2)
  val exmemHazardA = (io.exmemRd =/= 0.U && io.exmemRd === io.idRs1)
  val exmemHazardB = (io.exmemRd =/= 0.U && io.exmemRd === io.idRs2)
...
class PipelinedRV32Icore (BinaryFile: String) extends Module {
  ...
  var fwUnit = Module(new ForwardingUnit)
  ...
  fwUnit.io.idRs1     := idBarrier.io.rs1out
  fwUnit.io.idRs2     := idBarrier.io.rs2out
  fwUnit.io.exRd      := exBarrier.io.rdOut
  fwUnit.io.wbRd      := WB.io.rd
  fwUnit.io.wbWriteEn := WB.io.wbToRegFile.writeEn
  ...
  val forwardA = MuxLookup(fwUnit.io.forwardA, regFile.io.resp1.data, Seq(
    ...
    2.U -> memBarrier.io.resultOut // Forward from EXMEM
   // 3.U -> WB.io.result
  ))

  val forwardB = MuxLookup(fwUnit.io.forwardB, regFile.io.resp2.data, Seq(
    0.U -> regFile.io.resp2.data, // No forwarding
    1.U -> exBarrier.io.resOut,    // Forward from EX
    2.U -> memBarrier.io.resultOut // Forward from EXMEM
    //3.U -> WB.io.result
  ))
  ...
  EX.io.operandA := forwardA
  EX.io.operandB := forwardB
