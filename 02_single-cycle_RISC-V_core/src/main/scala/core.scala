...
class RV32Icore (BinaryFile: String) extends Module {
  ...
  val zero = "b00000"
  regFile(0) := 0.U // hard wiring register x0 to 0 (why is it red?)
  //regFile.write(0.U, 0.U) // is this the way to write and re
  ...
  val imm = instr(31, 20) // if we have I type instruction


  val isADD  = (opcode === "b0110011".U && funct3 === "b000".U && funct7 === "b0000000".U) // add
  val isSUB  = (opcode === "b0110011".U && funct3 === "b000".U && funct7 === "b0100000".U) // sub
  val isSLL  = (opcode === "b0110011".U && funct3 === "b001".U && funct7 === "b0000000".U) // Shift Left Logical
  val isSLT  = (opcode === "b0110011".U && funct3 === "b010".U && funct7 === "b0000000".U) // Shift Left Arithmetic
  ...
  val isXOR  = (opcode === "b0110011".U && funct3 === "b100".U && funct7 === "b0000000".U) // XOR
  val isOR   = (opcode === "b0110011".U && funct3 === "b110".U && funct7 === "b0000000".U) // OR
  val isAND  = (opcode === "b0110011".U && funct3 === "b111".U && funct7 === "b0000000".U) // AND
  ...
  when(isADDI) { 
    aluResult := operandA + imm // not really sure about that
  }.elsewhen(isADD) {                           
    ...
    aluResult := operandA << operandB(4, 0) // shifting left is same for arithmetic and logical
  }.elsewhen(isSRL) {
    aluResult := operandA >> operandB(4, 0) // restrict operandA
  }.elsewhen(isSRA) {
//    val signBit = operandA(31)
//    val shifted1 = operandA >> operandB(4,0)
//    val shiftAmount = operandB(4,0)
//    val shiftAmountInt = shiftAmount.litValue
   // print("shift amount :::" + shiftAmountInt)
    aluResult := (operandA.asSInt >> operandB(4, 0)).asUInt // restrict operandA


   // aluResult := Cat(Fill(shiftAmountInt, signBit), shifted1(31-shiftAmountInt, 0))
  }.otherwise {
    aluResult := 666.U // maybe put here nop (how to implement)
